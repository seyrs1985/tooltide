...
- 基线(2026-09-26 R154 审计,R153 后):399 工具页(i18n 口径);check_site 405 文件 31512 检查 0 失败;i18n 审计 399 页 335 键×9 语言;收入 $0.00(DEMO)。**规模口径以当轮 check_site 输出为准**(build 输出的页数与 STATUS 口径不同)。
...
- 查重命令模板(R150 教训:只跑单形态漏掉早期双引号页,sitemap 去重闸门兜底拦下):grep -oE "['\"]slug['\"]: ['\"][a-z0-9-]+" pages.py | grep -i 关键词 —— 一条命令同时抓两种引号形态。
...
- pull 必须在任何本地编辑之前执行:审计名单/脚本这类小改动先改后 pull,会被自己的未提交改动挡住(R161/R167 两次实证)——轮内顺序固定为 pull→改→闸门→部署。
