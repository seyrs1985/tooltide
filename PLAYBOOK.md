...
- 留存 R1 遗留:.ics 导出已落地试点(R140 turkey-thaw,模式=date对象存作用域+String.fromCharCode造CRLF+data:URI下载;验证后按 date规划器族逐页推广);每日内容钩子、GSC 长尾(等真数)仍开;旧候选「换算页最近使用复用条」已由站级 RECENT_STRIP_JS 落地移除(R129 审计)
