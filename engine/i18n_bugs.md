...
- 品牌更名(2026-09-26)|全站|—|tooldune.com新域名启用后用户要求品牌同步更名:ToolTide→ToolDune全站替换368处(config/build/tools/pages/i18n表+gen注释;大小写敏感保护小写tooltide的URL/仓库名/邮箱;GA4/GSC/adsense ID不动;功能邮箱tooltide.contact@gmail.com保留)|curl实证首页ToolDune×2+/旧名残留0|已修R31+线上复验(402→zh 411键;检查23840/0;部署IndexNow分批200)
- BUG-013续11|全站高频裸span
- 复验(2026-09-26)|case-converter|pt|case.resultlbl接线在服务HTML中实证(配合品牌更名R31同页验证ToolDune标识)|curl直连|已修R26-续+线上复验通过(R33待复核项之case页部分闭环;camelCase等4专名范围外维持)
- 复验(2026-09-26)|firewood-calculator+pumpkin-pie-calculator|ja/pt|fw.*/ppie.*接线在服务HTML实证(fw.area/fw.cords/data-i18n各2处+garbage=0)——注意:并行工作者04:22的部署曾卷入旧构建导致接线短暂缺失,重建docs并推送后恢复(并发同树的构建/提交时序错位风险,已入教训)|所有译文语言|高频重复stat标签未接线TOP11(per month×4/total interest×3/you save×2/total paid×2/per year×2/per day×2/paintable area×2/full charge cost×2/day of year×2/cubic meters×2/words×2)|rg存量盘点|已修R20+部署(脚本化25处实例接lbl.permonth/peryear/perday+fin.totalinterest/totalpaid/yousave+paint.paintable+ev.fullcharge+date.dayofyear+conv.cubicmeters×10键×9语言,words复用既有wc.words;并行工作者卷带提交部署;长尾×1约300种留逐家族)
...
- BUG-013续17|slow-cooker-converter|ja|7处未接线(ovenmin/setting标签+in the slow cooker result-unit+the other setting/liquid in the recipe/dairy and seafood go in stat;slowcook.*7键×9语言静态接线;JS拼接串hours-on/cut-by-half/last-hour与note长文下轮TT化)+裸span错位3处当轮修复(garbage门二次实战)|IAB DOM探针|已修R45+线上复验(curl实证slowcook.ovenmin/dairy接线2处在服务HTML+garbage=0)
- BUG-013续18|pto-optimizer|ko|7处未接线(PTO days per year/Breaks you want标签+calendar days off chained result-unit+vs one naive block/weekends harvested/the Wednesday trick stat+Share my PTO math按钮+1处JS恢复;pt.*6键+share.share-my-ptomath×9语言)|IAB DOM探针|已修R46+线上复验(curl实证pto页6处data-i18n+garbage=0+i18n.js含pt键18处)
- BUG-013续19|deck-stain-calculator|ru|9处未接线(Deck area/Railing/Coats/Price per gallon标签+gallons of stain result-unit+total cost/brushing hours/redo cycle stat+Share my stain math按钮;deck.*8键+share.share-my-stainmath×9语言;其中total cost通用标签同模式命中mulch/hc2/cp三工具,语义通用可复用)|IAB DOM探针|已修R45+线上复验(curl实证deck.area/gallons接线2处在服务HTML+garbage=0)
...
- 2026-09-26 R34(07:15档,等旧实例收工后执行):bmi×es(BMI为通用缩写,范围外)/lorem×ko全净;b64×ja的output为嵌套span外层假阳性(dev.* ja接线此前已验证日文渲染)。零新BUG。
