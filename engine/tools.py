...
<div class="tool" id="tt-case">
  ...
  <div class="field"><label for="case-out"><span data-i18n="case.resultlbl">Result</span> <button class="btn btn-sm" id="case-copy" type="button" data-i18n="ui.copy">Copy</button></label>
...
hms.addEventListener('input',function(){
  ...
  if(v===null){out.textContent=T('secconv.usehms','(use h:mm:ss)');sIn.value='';lock=false;return;}
  out.textContent=v.toLocaleString((typeof window!=='undefined'&&window.ttLang)?window.ttLang():'en-US')+' '+T('secconv.seconds','seconds');sIn.value=v;
  lock=false;
});
})();</script>
"""
...
if(GOAL.value!=='custom'&&!P.value&&!C.value&&!F.value){var dv=(GOAL.value||'30,40,30').split(',');P.value=dv[0];C.value=dv[1];F.value=dv[2];}
calc();
...
HRZONE = """<div class="tool" id="tt-hz">
  <div class="fields">
    ...
    <div class="field"><label for="hz-r" data-i18n="hz.rest">Resting heart rate</label><input type="number" id="hz-r" min="30" max="120" placeholder="60"></div>
  ...
  <div class="result" aria-live="polite" aria-atomic="true"><span class="result-num" id="hz-out">–</span><span class="result-unit" data-i18n="hz.maxhr">max heart rate</span></div>
